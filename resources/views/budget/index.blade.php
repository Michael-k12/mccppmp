<x-layouts.app :title="'Budget'">
    ...
    <div class="container mx-auto px-4 py-8 space-y-8">
        ...
        @if ($activeBudget)
            <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 px-5 py-4 rounded-xl shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <form id="deleteSelectedForm" method="POST" action="{{ route('budget.deleteSelected') }}" class="w-full sm:w-auto flex-shrink-0">
                @csrf
                @method('DELETE')
                <button type="submit" id="deleteSelectedBtn" class="bg-red-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-red-600 transition hidden w-full sm:w-auto">
                    Delete Selected
                </button>
            </form>
        </div>
    ...
    <style>
        /* Buttons */
        .start-proposal-btn { background: #10b981; color: #fff; padding: 8px 16px; border-radius: 10px; font-weight: 600; transition: 0.2s; }
        .start-proposal-btn:hover { background: #059669; transform: scale(1.05); }
        .save-budget-btn { background: #2563eb; color: #fff; padding: 12px; border-radius: 10px; font-weight: 600; transition: 0.2s; }
        .save-budget-btn:hover { background: #1e40af; transform: scale(1.05); }
        .end-proposal-btn { background: #ef4444; color: #fff; padding: 8px 16px; border-radius: 8px; font-weight: 500; transition: 0.2s; }
        .end-proposal-btn:hover { background: #dc2626; transform: scale(1.05); }

        /* Inputs */
        .modern-input { border: 1px solid #d1d5db; border-radius: 10px; padding: 10px 14px; width: 100%; font-size: 15px; background: #f9fafb; }
        .modern-input:focus { outline: none; border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37,99,235,0.25); background: #fff; }
    ...
    <script>
        ...
        function confirmEndProposal(){ Swal.fire({title:'Are you sure?',text:"This will end the current proposal!",icon:'warning',showCancelButton:true,confirmButtonColor:'#ef4444',cancelButtonColor:'#6b7280',confirmButtonText:'Yes, end it!',cancelButtonText:'Cancel'}).then(r=>{if(r.isConfirmed)document.getElementById('endProposalForm').submit();}); }
        function validateYear(input){ if(input.value.length>4) input.value=input.value.slice(0,4); }
        function formatNumberInput(input){ let v=input.value.replace(/[^0-9.]/g,''); let p=v.split('.'); if(p.length>2)v=p[0]+'.'+p[1]; if(p[1])p[1]=p[1].slice(0,2); input.value=p[0].replace(/\B(?=(\d{3})+(?!\d))/g,',')+(p[1]?'.'+p[1]:''); }
        document.getElementById('budgetForm').addEventListener('submit',()=>{ document.getElementById('amount').value=document.getElementById('amount').value.replace(/,/g,''); });

        const selectAllCheckbox=document.getElementById('selectAll'); const budgetCheckboxes=document.querySelectorAll('.budget-checkbox'); const deleteBtn=document.getElementById('deleteSelectedBtn');
        selectAllCheckbox.addEventListener('change',()=>{ budgetCheckboxes.forEach(cb=>cb.checked=selectAllCheckbox.checked); toggleDeleteBtn(); });
        budgetCheckboxes.forEach(cb=>cb.addEventListener('change',toggleDeleteBtn));
        function toggleDeleteBtn(){ deleteBtn.classList.toggle('hidden',!Array.from(budgetCheckboxes).some(cb=>cb.checked)); }
            ...
            if(!anyChecked){ e.preventDefault(); alert('Please select at least one budget to delete.'); }
                ...
                Swal.fire({
                    ...
                    if(r.isConfirmed){
                        ...
                        budgetCheckboxes.forEach(cb=>{ if(cb.checked){ const input=document.createElement('input'); input.type='hidden'; input.name='selected[]'; input.value=cb.value; form.appendChild(input); }});
                        form.submit();
                    }
                });
            }
        ...
        document.addEventListener("DOMContentLoaded",()=>{
            @if(session('success')) Swal.fire({toast:true,position:'top-end',icon:'success',title:@json(session('success')),showConfirmButton:false,timer:2500}); @endif
            @if(session('error')) Swal.fire({toast:true,position:'top-end',icon:'error',title:@json(session('error')),showConfirmButton:false,timer:3000}); @endif
            @if(session('warning')) Swal.fire({toast:true,position:'top-end',icon:'warning',title:@json(session('warning')),showConfirmButton:false,timer:3000}); @endif
            @if(session('info')) Swal.fire({toast:true,position:'top-end',icon:'info',title:@json(session('info')),showConfirmButton:false,timer:2500}); @endif
        });
    </script>
